...
- **应当** 用户级别：最大会话数、最大 workspace 总大小、每日 LLM 调用次数；
- **应当** 沙箱级别：CPU/内存/磁盘上限；
- **MVP** 用配置文件硬编码即可，不做自服务管理。
...
flowchart LR
    ...
    subgraph Data[数据层]
        PG[(PostgreSQL<br/>用户/会话/消息)]
        Vec[(PgVector<br/>RAG 向量库)]
        Obj[(对象存储 / 本地卷<br/>workspace)]
        Cache[(Redis<br/>Session/限流/锁)]
    end
...
| **PgVector** | RAG 向量库（按用户/会话 namespace） |
| **Redis** | 短期态：在线状态、限流计数、分布式锁、**用户并发 Agent 数计数** |
...
### 7.2 向量库 namespace 设计

- 单表 `vector_store`，通过 metadata `{ userId, scope: "user"|"session", sessionId? }` 过滤；
- 检索时**强制注入** `userId = current_user` 的过滤条件，由后端拼接，不信任前端；
- 后续可拆为 `vector_store_user_<id>` 多表（按需）。
