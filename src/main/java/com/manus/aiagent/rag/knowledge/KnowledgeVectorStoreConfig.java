...
public class KnowledgeVectorStoreConfig {
    ...
    VectorStore knowledgeVectorStore(JdbcTemplate jdbcTemplate, EmbeddingModel dashscopeEmbeddingModel) {
        ...
        // 加载文档
        List<Document> documentList = knowledgeDocumentLoader.loadDocuments();
        // 自主切分文档
//        List<Document> splitDocuments = myTokenTextSplitter.splitCustomized(documentList);
        ...
        boolean locked = acquireLock(jdbcTemplate, 2026031701L);
        try {
            cleanupDuplicateKnowledgeRows(jdbcTemplate);
            syncKnowledgeVectorStore(jdbcTemplate, enrichedDocuments);
            ...
            if (locked) {
                releaseLock(jdbcTemplate, 2026031701L);
            }
        }
    ...
    private boolean acquireLock(JdbcTemplate jdbcTemplate, long lockKey) {
        ...
        try {
            ...
            log.warn("获取 knowledge 启动锁失败，继续无锁执行: {}", e.getMessage());
            return false;
        }
    }
    ...
    private void releaseLock(JdbcTemplate jdbcTemplate, long lockKey) {
        ...
        try {
            ...
            log.warn("释放 knowledge 启动锁失败: {}", e.getMessage());
        }
    }

    private void cleanupDuplicateKnowledgeRows(JdbcTemplate jdbcTemplate) {
        if (jdbcTemplate == null) return;
        jdbcTemplate.update("""
                WITH ranked AS (
                    SELECT ctid,
                    ...
                    WHERE metadata->>'source'='knowledge' OR COALESCE(metadata->>'source','')=''
                )
    ...
    private void syncKnowledgeVectorStore(JdbcTemplate jdbcTemplate, List<Document> currentDocs) {
        ...
        List<String> dbFilenames = jdbcTemplate.queryForList(
                "SELECT DISTINCT metadata->>'filename' FROM public.vector_store WHERE COALESCE(metadata->>'source','') IN ('knowledge','')",
                String.class
        );
        for (String dbFilename : dbFilenames) {
            ...
            if (!currentFilenames.contains(dbFilename)) {
                int deleted = jdbcTemplate.update(
                        "DELETE FROM public.vector_store WHERE metadata->>'source'='knowledge' AND metadata->>'filename'=?",
                        dbFilename
                );
                if (deleted > 0) {
                    log.info("knowledge 向量库已删除缺失文件 {} 的向量记录数：{}", dbFilename, deleted);
                }
            }
        }
        ...
        for (String filename : currentFilenames) {
            ...
            List<String> dbHashes = jdbcTemplate.queryForList(
                    "SELECT metadata->>'chunk_hash' FROM public.vector_store WHERE COALESCE(metadata->>'source','') IN ('knowledge','') AND metadata->>'filename'=?",
                    String.class,
                    filename
            );
            for (String dbHash : dbHashes) {
                if (dbHash == null || dbHash.isBlank()) {
                    jdbcTemplate.update(
                            "DELETE FROM public.vector_store WHERE metadata->>'source'='knowledge' AND metadata->>'filename'=? AND (metadata->>'chunk_hash' IS NULL OR metadata->>'chunk_hash'='')",
                            filename
                    );
                    continue;
                }
                if (!currentHashes.contains(dbHash)) {
                    jdbcTemplate.update(
                            "DELETE FROM public.vector_store WHERE metadata->>'source'='knowledge' AND metadata->>'filename'=? AND metadata->>'chunk_hash'=?",
                            filename,
                            dbHash
                    );
                }
    ...
    private boolean isKnowledgeChunkEmbedded(JdbcTemplate jdbcTemplate, Document doc) {
        ...
        Integer count = jdbcTemplate.queryForObject(
                "SELECT COUNT(1) FROM public.vector_store WHERE COALESCE(metadata->>'source','') IN ('knowledge','') AND metadata->>'filename'=? AND metadata->>'chunk_hash'=?",
                Integer.class,
                filename,
                chunkHash
        );
